...
data_hh_input = np.load(prefix + "/Helmholtz_inputs.npy")
data_hh_output = np.load(prefix + "/Helmholtz_outputs.npy")
...
# whiten the data
data_hh_input = data_hh_input - np.mean(data_hh_input, axis=2)[:,:, None]
data_hh_output = data_hh_output - np.mean(data_hh_output, axis=2)[:,:, None]


n_train = cfg.M  # N
# n_test = 40000 - n_train
n_test = cfg.M
X_train = data_hh_input[:, :, :n_train]
X_test = data_hh_input[:, :, n_train:n_train+n_test]
Y_train = data_hh_output[:, :, :n_train]
Y_test = data_hh_output[:, :, n_train:n_train+n_test]
X_train_flat = np.reshape(X_train, (-1, n_train)) # X
Y_train_flat = np.reshape(Y_train, (-1, n_train))  # Y
X_test_flat = np.reshape(X_test, (-1, n_test))  # \tilde{X}
Y_test_flat = np.reshape(Y_test, (-1, n_test))  # \tilde{Y}
...
# scheduler = MultiStepLR(optimizer, milestones=[100, 200, 300, 400, 500], gamma=0.5)
#
for ep in range(num_epoches):
    ...
    if ep % ep_predict == 0:
        ...
        writer.add_scalar("pred/loss", average_relative_error, ep)
        writer.add_scalar("pred2/loss", average_relative_error2, ep)
