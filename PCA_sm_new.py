...
data_navier_input = np.load(prefix + "/StructuralMechanics_inputs.npy")
data_navier_output = np.load(prefix + "/StructuralMechanics_outputs.npy")
...
parser.add_argument('--device', type=int, default=2, help="index of device")
...
X_train_flat = X_train.reshape(-1, n_train)  # X
Y_train_flat = Y_train.reshape(-1, n_train)  # Y
X_test_flat = X_test.reshape(-1, n_test)  # \tilde{X}
Y_test_flat = Y_test.reshape(-1, n_test)  # \tilde{Y}
...
for ep in range(num_epoches):
    ...
    writer.add_scalar("test/loss", test_l2_step / n_test, ep)
    if ep % ep_predict == 0:
